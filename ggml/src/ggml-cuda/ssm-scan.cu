...
#pragma unroll
            for (int j = 0; j < ((w >> 1) * splitH + d_state - 1) / d_state; j++) {
                const int k = (threadIdx.x % (w >> 1)) + d_state * (threadIdx.x / (w >> 1)) +
...
#pragma unroll
        for (int j = 0; j < splitH / (d_state / WARP_SIZE); j++) {
            float y = stateC[(threadIdx.x % WARP_SIZE) + d_state * (threadIdx.x / WARP_SIZE) +
                             j * d_state * (d_state / WARP_SIZE)];
            y = warp_reduce_sum(y);
        ...
        if (K > 1 && slot > 0 && slot < K) {
            float * s_snapshot_block = (float *) ((char *) dst + s_off + (slot * gridDim.y + seq_idx) * src0_nb3 + head_idx * src0_nb2 + head_off * d_state);
#pragma unroll
            for (int j = 0; j < splitH; j++) {
                s_snapshot_block[j * d_state + threadIdx.x] = state[j];
            }
        }
    }
...
static void ssm_scan_f32_cuda(const float * src0, const float * src1, const float * src2, const float * src3,
    ...
    if (src3_nb1 == sizeof(float)) {
        ...
        if (d_state == 128) {
            ...
            ggml_cuda_kernel_launch(ssm_scan_f32_group<splitH, 128>, launch_params,
            ...
            const dim3 blocks((n_head * head_dim + (splitH - 1)) / splitH, n_seq, 1);
            const ggml_cuda_kernel_launch_params launch_params = ggml_cuda_kernel_launch_params(blocks, threads, 0, stream);
            ggml_cuda_kernel_launch(ssm_scan_f32_group<splitH, 256>, launch_params,
                    src0, src1, src2, src3, src4, src5, src6, dst,
                    src0_nb2, src0_nb3, src1_nb2, src1_nb3, src2_nb1, src2_nb2, src3_nb1,
                    src4_nb2, src4_nb3, src5_nb2, src5_nb3, s_off, n_head, head_dim, n_group, n_tok, K);
        } else {
