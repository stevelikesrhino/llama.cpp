...
bool llama_model_base::load_tensors(llama_model_loader & ml) {
    ...
    auto use_cuda_nvfp4_repack = [](ggml_backend_dev_t dev) {
        ...
        using use_nvfp4_repack_t = bool (*)(ggml_backend_dev_t);
        auto fn = (use_nvfp4_repack_t) ggml_backend_reg_get_proc_address(
            reg, "ggml_backend_cuda_device_uses_nvfp4_repack");
    ...
    auto attach_nvfp4_scales = [](ggml_tensor * weight, ggml_tensor *& weight_scale, ggml_tensor * input_scale, bool use_cuda_nvfp4_scales) {
        ...
        // Sidecar scales are optional; ordinary NVFP4 tensors are self-contained.
    ...
    for (int il = 0; il < n_layer_all; ++il) {
        ...
        const bool use_cuda_nvfp4_scales = use_cuda_nvfp4_repack(pimpl->dev_layer[il].dev);
        ...
        attach_nvfp4_scales(layer.nextn.eh_proj,          layer.nextn.eh_proj_s,          layer.nextn.eh_proj_in_s,          use_cuda_nvfp4_scales);
        attach_nvfp4_scales(layer.nextn.shared_head_head, layer.nextn.shared_head_head_s, layer.nextn.shared_head_head_in_s, use_cuda_nvfp4_scales);
    }
    ...
    attach_nvfp4_scales(output, output_s, output_in_s, use_cuda_nvfp4_repack(pimpl->dev_output.dev));
    ...
    for (int il = 0; il < n_layer_all; ++il) {
        ...
        cache_nvfp4_scales(layer.nextn.eh_proj);
        cache_nvfp4_scales(layer.nextn.shared_head_head);
    }
    cache_nvfp4_scales(output);
