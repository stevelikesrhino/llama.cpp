...
static gguf_context_ptr get_gguf_ctx(const llm_arch arch, const bool moe, const bool nvfp4_merged = false) {
...
static void test_merged_nvfp4_scale_loading(const size_t seed) {
    gguf_context_ptr gguf_ctx = get_gguf_ctx(LLM_ARCH_QWEN35MOE, true, true);
...
int main(int argc, char ** argv) {
    ...
    try {
        ...
        if (arch == LLM_ARCH_UNKNOWN || arch == LLM_ARCH_QWEN35MOE) {
            test_merged_nvfp4_scale_loading(seed);
        }
